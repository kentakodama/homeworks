...
class Play
  ...
  def self.find_by_title(title)
    raise 'Not in the database' unless @id
    result = PlayDBConnection.instance.execute(<<-SQL, title)
      SELECT *
      FROM plays
      WHERE title = ?
    SQL
    result.map { |datum| Play.new(datum) }
  end
...
class Playwright
  ...
  def get_plays(playwright)
    result = PlayDBConnection.instance.execute(<<-SQL, @name, @birth_year, @id)
      SELECT *
      FROM playwrights
        LEFT JOIN plays
          ON playwright.id = plays.playwright_id
          WHERE playwright.name = ?
    SQL
    result.map {|datum| playwright.new(datum) }
  end
